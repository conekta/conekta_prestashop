...
{if $conekta_order.valid == 1 }
		<div class="conf confirmation">{l s='Pago Exitoso, el pago ha sido aprobado y el pedido se ha guardado con la referencia ' mod='conektaprestashop'} <b>{$conekta_order.reference|escape:html:'UTF-8'}</b>.</div>
{else}
		...
		({l s='Your Order\'s Reference:' mod='conektaprestashop'} <b>{$conekta_order.reference|escape:html:'UTF-8'}</b>)</div>
{/if}
{/if}
...
<p> Informacion para realizar el pago por medio de SPEI: <p>
                ...
                <br>{$conekta_order.receiving_account_number|escape:html:'UTF-8'}
                <br /><br /><span>
...
<p> Informacion para realizar el pago por medio de BANORTE: <p>
                <br><br><b>{l s='Monto:' mod='pagoefectivo'}</b> $ {$conekta_order.amount } {$conekta_order.currency }
                <br><br><b>{l s='Referencia:' mod='pagoefectivo'}</b>
                <br>{$conekta_order.reference|escape:html:'UTF-8'}
                <br><br><b>{l s='Número de servicio:' mod='pagoefectivo'}</b>
                <br>{$conekta_order.service_number|escape:html:'UTF-8'}
                <br><br><b>{l s='Nombre de servicio:' mod='pagoefectivo'}</b>
                <br>{$conekta_order.service_name|escape:html:'UTF-8'}
                <br /><br /><span>
