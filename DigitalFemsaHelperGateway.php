 ...
 * @see     https://conekta.com/
 */
class DigitalFemsaHelperGateway
{
    /**
     * Add tax lines
     *
        ...
        if (isset($taxlines)) {
            foreach ($taxlines as $tax) {
                array_push(
                    $tax_lines['tax_lines'],
                    [
                        'description' => DigitalFemsaHelperGateway::removeSpecialCharacter($tax['description']),
                        'amount' => $tax['amount'],
                    ]
                );
            }
            ...
            foreach ($shippingLines as $shipping) {
                array_push(
                    ...
                    [
                        ...
                        'tracking_number' => DigitalFemsaHelperGateway::removeSpecialCharacter($shipping['tracking_number']),
                        'carrier' => DigitalFemsaHelperGateway::removeSpecialCharacter($shipping['carrier']),
                        'method' => DigitalFemsaHelperGateway::removeSpecialCharacter($shipping['method']),
                    ]
                );
            }
        }
    ...
    public static function validateSubscrition($items)
    {
        if (count($items) > 1) {
            ...
            return FemsaDigitalDatabase::isProductSubscription($items[0]['id_product']);
        }
    ...
    {
        if (count($items) > 1) {
            return DigitalFemsaHelperGateway::validateItemsProduct($items);
        } elseif ($items[0]['cart_quantity'] > 1) {
    ...
    public static function validateItemsProduct($items)
    {
        ...
        while (count($items) > $i && $non_subscription) {
            if (FemsaDigitalDatabase::isProductSubscription($items[$i]['id_product'])) {
    ...
    {
        $retorno = DigitalFemsaHelperGateway::validateSubscrition($items);
        ...
        } elseif (DigitalFemsaHelperGateway::validateItemsProduct($items)) {
            return [
