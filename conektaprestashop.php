...
class ConektaPrestashop extends PaymentModule
{
  ...
  public function hookPaymentReturn($params)
  {
    if (!$this->active)
      return;
            ...
            } else if ($conekta_transaction_details['reference']) {
            $this->smarty->assign('spei', true);
            $this->smarty->assign('conekta_order', array('clabe' => $conekta_transaction_details['reference'], 'amount'=>$conekta_transaction_details['amount'], 'currency'=>$conekta_transaction_details['currency']));
  ...
  public function processPayment($token, $type, $monthly_installments)
  {
    ...
    try
    {
            ...
            $checkout = Module::getInstanceByName('conektaprestashop');
            $checkout->extra_mail_vars = array( '{clabe}' => (string)$reference);
